				...
				$.ajax({
					...
					success: function (response) {
						...
						$.each(result, function (i, item) {
							var eventColor = '#74be85';  // Default color (blue for appointments)

							// Assign green color for "booking" status
							if (result[i].status === 'booking') {
								eventColor = '#5a65c3';  // Green for booking requests
							}
    ...
    status: result[i].status || 'N/A'
						...
						var calendar = $('#calendar').fullCalendar({
							...
							dayRender: function (date, cell) {
								if (date.isSame(new Date(), 'day')) { // Check if the date is today
									cell.css("background-color", "#f5eded");  // Change background color (yellow example)
								}
							},
